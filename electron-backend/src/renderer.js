...
socket.on('private message', ({ from, message }) => {
  displayMessage(message, 'private', from);
});
...
function displayMessage(message, type = 'normal', from = null) {
  ...
  if (type === 'private') {
    ...
    if (from === socket.id) {
      messageElement.textContent = `You (private to ${selectedUser}): ${message}`;
    } else {
      messageElement.textContent = `Private from ${from}: ${message}`;
    }
  } else {
    // Group message
    messageElement.textContent = `${message}`;
